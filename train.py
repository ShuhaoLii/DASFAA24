...
def main(args, **model_kwargs):
    ...
    data = util.load_dataset(args.data, args.batch_size, args.batch_size, args.batch_size,args.horizon, args.target_range,n_obs=args.n_obs, fill_zeroes=args.fill_zeroes)
    scaler = data['scaler']
    aptinit, supports = util.make_graph_inputs(args, device)

    model = AGWNet.from_args(args, device, supports, aptinit, **model_kwargs)
    # print(model)
    if args.checkpoint:
    ...
    metrics = []
    best_model_save_path = os.path.join(args.save, 'PeMS_best_model_%s.pth' %args.horizon)
    ...
    # att1 = att[-1,-1,:,:]
    # att1 = pd.DataFrame(att1.cpu().detach().numpy()).to_csv('att.csv')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save', type=str, default='logs/PeMS', help='save path')
    ...
    #target range [num of lanes,x_min,y_min,x_max,y_max][4,2,2,4,4],global：0 [4,1,1,3,3]
    parser.add_argument('--target_range', type=int, default=[4,1,1,3,3], help='when target_range = 0 is global prediction')
