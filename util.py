...
def load_adj_PEMS(adjtype):
    A = pd.read_csv ("/root/Lane_level/AGWN/Data_process/data/PEMS_adj.csv", index_col=0)
    A = np.array(A)
...
def load_dataset(dataset_dir, batch_size, valid_batch_size, test_batch_size,horizon, target_range,n_obs=None, fill_zeroes=True):
    data = {}
    for category in ['train', 'val', 'test']:
        ...
        if n_obs is not None:
            ...
            data['y_' + category] = data['y_' + category][:, 0:int (horizon), :, :]
        if target_range != 0:
            for i in range(target_range[1],target_range[3]+1):
                target_lane = data['y_' + category][:, :, int(i*target_range[0] + target_range[2]):int(i*target_range[0]+ target_range[4])+1, :]
                target_lanes = target_lane
                if i > target_range[1]:
                    target_lanes = np.concatenate ((target_lanes, target_lane), axis=2)
            data['y_' + category] = target_lanes
...
def make_graph_inputs(args, device):
    ...
    #PEMS
    adj_mx = load_adj_HuanNan(args.adjtype)
...
def get_shared_arg_parser():
    ...
    parser.add_argument ('--horizon', type=int, default=6, help='')
    parser.add_argument('--data', type=str, default='Data_process/data/PEMS_lane40', help='data path')
    parser.add_argument('--adjdata', type=str, default='Data_process/data/sensor_graph/adj_mx.pkl',
                        help='adj data path')
    ...
    parser.add_argument('--in_dim', type=int, default=1, help='inputs dimension')#LA=2
